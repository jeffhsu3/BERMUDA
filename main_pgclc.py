...
if __name__ == '__main__':
    # Goal is to identify batch correct trajectories without destroying them 
    data_folder = sys.agrv[1]
    dataset_file_list = [
            'GSM4202927_ucla1.batch1.hESC.rawmatrix.txt.gz',
            'GSM4202933_ucla1.batch2.hESC.rawmatrix.txt.gz']
    cluster_similarity_file = data_folder + 'pancreas_metaneighbor.csv'
    code_save_file = data_folder + 'code_list.pkl'
    dataset_file_list = [sc.read(data_folder+f).transpose() for f in dataset_file_list]

    from IPython import embed
    embed()

    # Need to write something similar to MetaNeighbor or combat

    # read data
    # Calculate metaneighbors using scanpy leiden
